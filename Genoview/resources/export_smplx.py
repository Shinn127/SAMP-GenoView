...
UV_OBJ_PATH = Path("/Users/shinn/Downloads/smplx_uv_2023/smplx_uv_2023.obj")
OUTPUT_BIN_PATH = BASE_DIR / "SMPLX.bin"
OUTPUT_MAP_PATH = BASE_DIR / "SMPLX_vertex_source.npy"
...
def build_expanded_mesh(
    ...
    if not np.array_equal(template_faces, obj_vertex_faces):
        raise ValueError("OBJ face topology does not match the SMPL-X template faces.")
...
if __name__ == "__main__":
    ...
    joint_positions = output.joints[0, : len(SMPLX_JOINT_NAMES)].detach().cpu().numpy().astype(np.float32)

    if obj_vertices.shape[0] != template_vertices.shape[0]:
        raise ValueError("OBJ vertex count does not match the SMPL-X template vertex count.")
    ...
    print(f"  bone count: {len(SMPLX_JOINT_NAMES)}")
